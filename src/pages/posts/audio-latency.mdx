...
I recently watched [techless's video on audio latency](https://www.youtube.com/watch?v=okIpbu1tp_A), which opened my eyes to how horrible audio latency is on Windows. This led me down a rabbit hole, and eventually I managed to drop my audio latency by over {results.usb_dac_windows.latency - results.gpu_dp_monitor_windows.latency}ms, with it going down from {results.usb_dac_windows.latency}ms~ to around {results.gpu_dp_monitor_windows.latency}ms~ in tests. This is about as good as it can get from what I've seen.
