...
export const results = {
  gpu_dp_monitor_windows: {
    label: "gpu → displayport → monitor 3.5mm out\n(windows drivers)",
    latency: 35,
    focus: "latency",
  },
  ...
  gpu_extractor_optical_dac_windows: {
    label: "gpu → hdmi extractor → optical → dac\n(windows drivers)",
    latency: 41,
    focus: "quality",
  },
  usb_dac_windows: {
...
If you do choose this method, you'll still need to follow [the drivers section of this guide](#drivers). On my monitor, latency was {results.gpu_dp_monitor_nvidia.latency}ms~ before fixing drivers, and on my motherboard it was {results.mobo_realtek.latency}ms~.

<LatencyPlot
  data={Object.values(results).filter((result) => result.focus === "latency")}
  alt="Latency-focused results summary"
  height={265 /* @todo: dont use height, somehow set the height for each bar to a fixed amt so i dont need to tweak shit manually */}
/>
...
Finally, you likely can't use MSI mode when using them. See [#msi-mode](#msi-mode) for more details on why that matters.
...
<LatencyPlot data={Object.values(results)} height={370} />
