...
export const results = [
  ...
  { label: "usb → dac (windows drivers)", latency: 75, focus: "quality" },
];
...
<LatencyPlot data={results} alt="All of my results from testing" />

In this post I'll go over what I've changed in my setup to reduce latency, and the
same steps should apply for anyone who has similar components. If you have a different
DAC/audio interface/GPU/headphones etc there might be some difference, but for the
most part the same steps _should_ help you.
...
<LatencyPlot
  data={results.filter((result) => result.focus === "latency")}
...
<LatencyPlot
  data={results.filter((result) => result.focus === "quality")}
